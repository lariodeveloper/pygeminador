import re
import os
from customtkinter import CTkTextbox, CTkLabel, CTkFrame
...
class MyCodeTextBox(CTkTextbox):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs, fg_color='#e6e3da', text_color='black')
    ...
    def change_color(self, key=None):
        ...
        matches = re.finditer(r"\b({})\s+|".format("|".join(TAGS.keys())), text)

        # Remove todas as tags de realce existentes
        self.tag_remove(self, "1.0", "end")  # Clear all existing tags efficiently
